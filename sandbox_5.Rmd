...
```{r}
mod <- lm(log(avg_demand_mgd) ~ . + log(TemperatureMean), data=train)
summary(mod)
```
...
predictions <- mod%>% 
  predict(mgd_avg_2022)

compare <- data.frame(actual = mgd_avg_2022$avg_demand_mgd,
                      predicted = predictions)

error <- RMSE(predictions,mgd_avg_2022$avg_demand_mgd)
targ<-mgd_avg_2022$avg_demand_mgd
mmetric(targ,predictions,metrics_list)
```
...
predictions <- mod_PrevDayAll%>% 
  predict(PrevDayAll_2022)

compare <- data.frame(actual = PrevDayAll_2022$avg_demand_mgd,
                      predicted = predictions)

error <- RMSE(predictions, PrevDayAll_2022$avg_demand_mgd)
targ<-PrevDayAll_2022$avg_demand_mgd
mmetric(targ,predictions,metrics_list)
```
