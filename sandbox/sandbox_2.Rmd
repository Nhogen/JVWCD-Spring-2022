...
2020 Regression Model to Predict 2021 Data as a simple test
```{r}
# gather 2020 data
data2020 <- Avg3 %>% 
  filter(CalendarYear==2020) %>% 
...
#2020 model
mod2020 <- lm(avg_demand_mgd~., data = data2020)
summary(mod2020)
...
pred <- predict(mod2020, data2021)

# metrics
compare <- data.frame(actual = data2021$avg_demand_mgd,
                      predicted = pred)
compare

error <- RMSE(pred,data2021$avg_demand_mgd)
error

ttarg<-data2021$avg_demand_mgd
...
JFMA <- Avg3 %>% 
  filter(CalendarYear == 2021) %>% 
  filter(CalendarMonth == 1 | CalendarMonth == 2 | CalendarMonth == 3 | CalendarMonth == 4) %>%
  select(-date,
         -CalendarYear,
         -CalendarMonth,
         -CalendarDay)%>%
...
# predictions on q1 2021 data
JFMApred <- predict(mod2020, JFMA)

# metrics
compare <- data.frame(actual = JFMA$avg_demand_mgd,
                      predicted = JFMApred)
compare

error <- RMSE(JFMApred,JFMA$avg_demand_mgd)
error

ttarg <- JFMA$avg_demand_mgd
metrics_list = c("R2","MAE","MAPE","RMSE")
mmetric(ttarg, JFMApred, metrics_list)
```
...
y <- ts(train_Avg3$avg_demand_mgd, frequency = 365)

# remove avg_demand_mgd from train data
train_Avg3 <- train_Avg3 %>% 
  select(-avg_demand_mgd)

summary(train_Avg3)

# generate ts model
tslm_mod <- tslm(y ~., data = train_Avg3)
summary(tslm_mod)
...
pred_2022 <- forecast.tslm(tslm_mod, newdata = Avg3_2022)
summary(pred_2022)
...
mmetric(ttarg,predictions,metrics_list)
```
